...
const updatePage = async () => {
  ...
  const sugaryFruitArray = fruitsArray.filter(fruit => fruit.nutritions.sugar > 15);
  console.log(sugaryFruitArray);
  const sugaryFruitNames = sugaryFruitArray.map((fruit) => {
    return fruit.name;
  });

  // TODO: Create a new HTML element to display your data 
  const newElement = document.createElement('div');
  newElement.innerHTML = sugaryFruitNames.toString();
